...
@testset "3D Conformal Geometric Algebra" begin
    ...
    @testset "Conformal points" begin
        ...
        a = sum(randn(3) .* [v2, v3, v4])
        b = sum(randn(3) .* [v2, v3, v4])
    ...
    @testset "Circles" begin
        C1 = circle(point(1v2), point(1v3), point(-1v2))
        @test C1 == 2.0v235
        @test normalize(center(C1)) == O
        @test radius2(C1) == 1v

        C2 = circle(point(4v2), point(2v2 + 2v3), point(0v1))
        @test C2 == 4(-v123 + v235 + 4v135)
        @test normalize(center(C2)) == point(2v2)
        @test radius2(C2) == 4v
    end

    @testset "Spheres" begin
        S1 = sphere(point(1v2), point(1v3), point(1v4), point(-1v2))
        @test S1 == 2.0v2345
        @test normalize(center(S1)) == O
        @test radius2(S1) == 1v

        S2 = sphere(point(0v1), point(4v2), point(2v2 + 2v3), point(2v2 + 2v4))
        @test normalize(center(S2)) == point(2v2)
        @test radius2(S2) == 4v
    end
end
